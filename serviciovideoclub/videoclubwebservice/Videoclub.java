...
public class Videoclub {
	...
	public void insertarPeliculas(String nombre, String genero, int cantidad, double preciopordia, String contrasena)
			throws Exception {
		if (contrasena.equals(contraadmin)) {

			/*
			 * Se generan las peliculas con los atributos correspondientes de cada una,para
			 * ello usamos un bucle para ir rellenando la lista
			 */
	...
	public void DevolverPelicula(int idpeli, String numCuenta) throws Exception {
		...
		/*
		 * Variable utilizada para llevar la cuenta de la posición donde se encuentra en
		 * la lista la película que queremos devolver
		 */
		int indice = 0;
	...
	public void ingresar(String numCuenta, int cantidad) throws Exception {
		...
		for (int i = 0; i < cuentas.size(); i++) {
			if (((Cuenta) cuentas.get(i)).getNumCuenta().equals(numCuenta)) {
				...
				if (encontrado == true)
					System.out
							.println("Dinero en la cuenta " + numCuenta + " " + ((Cuenta) cuentas.get(i)).getBalance());
				i = cuentas.size(); // Para que no siga buscando
			}
		}
		if (encontrado == false)
